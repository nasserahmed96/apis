...
class DataAccessObject(object):
    ...
    def select(self, columns=None, conditions=None, table_name=''):
        ...
        :param table_name: A string represents the table name in the database
    ...
    def get_data_for_completer(self, column):
        ...
        """
        joining_table = "users"
        joining_column = "user_id"
        joining_query = f"SELECT {column} FROM {self.table_name} " \
                              f"INNER JOIN {joining_table} ON {self.table_name}.{joining_column}={joining_table}.id"
        column_values_result = self.execute_select_query(joining_query)
        column_values = []
